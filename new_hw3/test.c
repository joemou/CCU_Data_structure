...
int main() {
    ...
    int* NodeID = (int*)malloc(Nodes * sizeof(int));
    int* QuantumMemories = (int*)malloc(Nodes * sizeof(int));
    ...
    int* ReqId = (int*)malloc(Req * sizeof(int));
    int* ReqSrc_index = (int*)malloc(Req * sizeof(int));
    int* ReqDst_index = (int*)malloc(Req * sizeof(int));
    ...
    for (int i = 0; i < TimeSlots; i++) {
        free(carry[i]);
        free(carry_capacity[i]);
    }
    free(carry);
    free(carry_capacity);
    free(NodeID);
    free(QuantumMemories);
    free(ReqId);
    free(ReqSrc_index);
    free(ReqDst_index);
